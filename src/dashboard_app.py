...
from traffic_sign_audit_system.run_full_audit import run_full_audit
# ------------------ helpers ------------------
...
if "dashboard_has_results" not in st.session_state:
    st.session_state.dashboard_has_results = False
...
with st.sidebar:
    ...
    st.header("Traffic Sign Visibility Audit")
    run_sign_audit = st.checkbox("Run Sign Visibility Audit", value=False)

    audit_detector_model_path = st.text_input(
    "Audit Sign Detector Model Path",
    "runs/detect/runs/traffic_sign_phase1/traffic_sign_detector_fast_v1/weights/best.pt"
)

    audit_start_frame = st.number_input("Audit Start Frame", min_value=0, value=0, step=1)
    audit_end_frame = st.number_input("Audit End Frame (-1 = full video)", value=-1, step=1)
    audit_conf = st.slider("Audit Detector Confidence", 0.05, 0.9, 0.25, 0.05)
    audit_imgsz = st.select_slider("Audit imgsz", options=[512, 640, 800, 960], value=512)
    audit_process_every = st.slider("Audit Process Every N Frames", 1, 10, 1, 1)
...
"""
    ...
    for i in range(selected_num_frames):
        ...
        if not np.isnan(distance_since_last_sign_m):
            put_text_outline(
                annotated,
                f"Distance traveled since sign: {distance_since_last_sign_m:.1f} m",
                (20, 70),
                scale=0.8
            )
...
### Visibilty sign Audit
    # =========================================================
    # Traffic Sign Visibility Audit Report (MAIN PAGE)
    # =========================================================
    if run_sign_audit:
        st.write("---")
        st.header("Traffic Sign Visibility Audit Report")

        audit_status_box = st.empty()
        audit_status_box.info("Running full traffic sign visibility audit pipeline...")

        try:
            audit_result = run_full_audit(
                video_path=video_path,
                telemetry_csv_path=telemetry_path,
                detector_model_path=audit_detector_model_path,
                output_dir="outputs/traffic_sign_audit_system",
                start_frame=int(audit_start_frame),
                end_frame=int(audit_end_frame),
                conf_thres=float(audit_conf),
                imgsz=int(audit_imgsz),
                process_every=int(audit_process_every),
                tracker_cfg="bytetrack.yaml",
                context_scale=3.0,
            )

            audit_status_box.success("Traffic sign visibility audit completed successfully.")

            report_csv = audit_result["report_csv"]
            recognition_csv = audit_result["recognition_csv"]
            visibility_csv = audit_result["visibility_csv"]
            location_csv = audit_result["location_csv"]

            report_df = pd.read_csv(report_csv) if os.path.exists(report_csv) else pd.DataFrame()
            recognition_df = pd.read_csv(recognition_csv) if os.path.exists(recognition_csv) else pd.DataFrame()
            visibility_df = pd.read_csv(visibility_csv) if os.path.exists(visibility_csv) else pd.DataFrame()
            location_df = pd.read_csv(location_csv) if os.path.exists(location_csv) else pd.DataFrame()
            ...
            total_signs = len(report_df)

            if "issue" in report_df.columns:
                signs_with_issues = int((report_df["issue"].astype(str) != "clear_visible").sum())
            else:
                signs_with_issues = 0

            if "severity" in report_df.columns:
                high_severity = int((report_df["severity"].astype(str).str.lower() == "high").sum())
            else:
                high_severity = 0

            if "sign_type" in report_df.columns:
                unique_sign_types = int(report_df["sign_type"].astype(str).nunique())
            else:
                unique_sign_types = 0

            c1, c2, c3, c4 = st.columns(4)
            c1.metric("Total Signs", total_signs)
            c2.metric("Signs With Issues", signs_with_issues)
            c3.metric("High Severity", high_severity)
            c4.metric("Unique Sign Types", unique_sign_types)

            # -------------------------
            # Final table you asked for
            # -------------------------
            st.subheader("Final Traffic Sign Audit Report")
            ...
            if not report_df.empty:
                ...
                st.warning("Final traffic sign audit report is empty.")
            ###
                        # =========================================================
            # Unique Problem Sign Summary + Gallery
            # =========================================================
            st.subheader("Problem Sign Summary")

            issue_df = report_df.copy()

            # keep only real problematic signs
            if "sign_type" in issue_df.columns:
                issue_df = issue_df[issue_df["sign_type"].astype(str).str.lower() != "not_sign"]

            if "issue" in issue_df.columns:
                issue_df = issue_df[issue_df["issue"].astype(str).str.lower() != "clear_visible"]

            if issue_df.empty:
                st.success("No problematic signs found in this selected audit window.")
            else:
                # severity rank for sorting
                sev_rank_map = {"high": 3, "medium": 2, "low": 1, "none": 0}
                issue_df["sev_rank"] = issue_df["severity"].astype(str).str.lower().map(sev_rank_map).fillna(0)

                # create grouping key to merge repeated track ids of same sign
                if "latitude" in issue_df.columns and "longitude" in issue_df.columns:
                    issue_df["lat_round"] = pd.to_numeric(issue_df["latitude"], errors="coerce").round(4)
                    issue_df["lon_round"] = pd.to_numeric(issue_df["longitude"], errors="coerce").round(4)
                    issue_df["sign_group_key"] = (
                        issue_df["sign_type"].astype(str) + "|" +
                        issue_df["issue"].astype(str) + "|" +
                        issue_df["lat_round"].astype(str) + "|" +
                        issue_df["lon_round"].astype(str)
                    )
                else:
                    # fallback if GPS not present
                    issue_df["frame_bucket"] = (pd.to_numeric(issue_df["frame"], errors="coerce") // 30).fillna(-1).astype(int)
                    issue_df["sign_group_key"] = (
                        issue_df["sign_type"].astype(str) + "|" +
                        issue_df["issue"].astype(str) + "|" +
                        issue_df["frame_bucket"].astype(str)
                    )

                # keep best row per grouped sign
                issue_df["quality_score_num"] = pd.to_numeric(issue_df.get("quality_score", 0), errors="coerce").fillna(0)
                issue_df = issue_df.sort_values(
                    by=["sev_rank", "quality_score_num", "frame"],
                    ascending=[False, False, True]
                )

                unique_issue_df = issue_df.drop_duplicates(subset=["sign_group_key"], keep="first").reset_index(drop=True)

                # summary counts
                s1, s2, s3 = st.columns(3)
                s1.metric("Unique Problem Signs", len(unique_issue_df))
                s2.metric("High Severity Signs", int((unique_issue_df["severity"].astype(str).str.lower() == "high").sum()))
                s3.metric("Unique Problem Types", int(unique_issue_df["issue"].astype(str).nunique()))

                # compact summary table
                st.write("### Unique Problem Signs Table")
                summary_cols = [
                    c for c in [
                        "frame",
                        "sign_type",
                    ...
                    if c in unique_issue_df.columns
                ]
                st.dataframe(unique_issue_df[summary_cols], use_container_width=True, hide_index=True)

                # =====================================================
                # Problem Sign Image Gallery
                # =====================================================
                st.write("### Problem Sign Gallery")

                # join with recognition to get crop/context paths
                recog_for_merge = recognition_df.copy()
                merge_cols = [c for c in ["track_id", "frame", "crop_path", "context_path"] if c in recog_for_merge.columns]
                recog_for_merge = recog_for_merge[merge_cols].drop_duplicates()

                gallery_df = unique_issue_df.merge(
                    recog_for_merge,
                    on=["track_id", "frame"],
                    how="left"
                )

                for idx, r in gallery_df.iterrows():
                    st.write("---")

                    top1, top2 = st.columns([1.1, 1.4])

                    with top1:
                        st.write(f"**Problem Sign #{idx + 1}**")

                        crop_path = r.get("crop_path", None)
                        context_path = r.get("context_path", None)

                        img_c1, img_c2 = st.columns(2)

                        with img_c1:
                            st.write("**Sign Crop**")
                            if isinstance(crop_path, str) and crop_path and os.path.exists(os.path.abspath(crop_path)):
                                st.image(os.path.abspath(crop_path), use_container_width=True)
                            else:
                                st.info("Sign crop not found.")

                        with img_c2:
                            st.write("**Context Crop**")
                            if isinstance(context_path, str) and context_path and os.path.exists(os.path.abspath(context_path)):
                                st.image(os.path.abspath(context_path), use_container_width=True)
                            else:
                                st.info("Context crop not found.")

                    with top2:
                        st.write("**Details**")
                        st.write(f"**Frame:** {r.get('frame', 'NA')}")
                        st.write(f"**Sign Type:** {r.get('sign_type', 'NA')}")
                        st.write(f"**Issue:** {r.get('issue', 'NA')}")
                        st.write(f"**Severity:** {r.get('severity', 'NA')}")
                        st.write(f"**Priority:** {r.get('priority', 'NA')}")
                        st.write(f"**Recommended Action:** {r.get('recommended_action', 'NA')}")

                        lat_val = r.get("latitude", None)
                        lon_val = r.get("longitude", None)
                        if pd.notna(lat_val) and pd.notna(lon_val):
                            st.write(f"**Latitude:** {lat_val}")
                            st.write(f"**Longitude:** {lon_val}")

                        if "maps_link" in r and pd.notna(r["maps_link"]) and str(r["maps_link"]).strip() not in ["", "None", "nan"]:
                            st.markdown(f"**Map Link:** [Open Map]({r['maps_link']})")
            ...
            if os.path.exists(report_csv):
                with open(report_csv, "rb") as f:
                    st.download_button(
                        "⬇️ Download Traffic Sign Final Report CSV",
                        data=f.read(),
                        file_name="traffic_sign_final_report.csv",
                        mime="text/csv"
                    )

        except Exception as e:
            audit_status_box.error("Traffic sign visibility audit failed.")
            st.exception(e)
#### 

elif st.session_state.dashboard_has_results:
    st.success("Showing previously processed results.")

    # -------------------------
    # paths
    # -------------------------
    report_csv = "outputs/traffic_sign_audit_system/traffic_sign_final_report.csv"
    ...
    location_csv = "outputs/traffic_sign_audit_system/traffic_sign_with_location.csv"
    sign_events_csv = "outputs/speed_sign_events.csv"
    sign_raw_csv = "outputs/speed_sign_raw_detections.csv"
    out_path = "outputs/dashboard_annotated.mp4"

    # -------------------------
    # rebuild main dashboard report from saved files
    # -------------------------
    stable_sign_df = pd.read_csv(sign_events_csv) if os.path.exists(sign_events_csv) else pd.DataFrame()
    raw_sign_df = pd.read_csv(sign_raw_csv) if os.path.exists(sign_raw_csv) else pd.DataFrame()
    ...
    # -------------------------
    # rebuild traffic sign audit report if available
    # -------------------------
    report_df = pd.read_csv(report_csv) if os.path.exists(report_csv) else pd.DataFrame()
    recognition_df = pd.read_csv(recognition_csv) if os.path.exists(recognition_csv) else pd.DataFrame()
    visibility_df = pd.read_csv(visibility_csv) if os.path.exists(visibility_csv) else pd.DataFrame()
    ...
    # -------------------------
    # main report section (keep visible after download)
    # -------------------------
    st.write("## Final Report")

    # fallback summary from saved files
    if not report_df.empty and "severity" in report_df.columns:
        high_issue_count = int((report_df["severity"].astype(str).str.lower() == "high").sum())
    else:
        high_issue_count = 0

    verdict_text = "SAFE"
    if high_issue_count > 0:
        verdict_text = "MODERATE_RISK"

    c1, c2, c3, c4 = st.columns(4)
    c1.metric("Telemetry Source", "gps_spd_3d / gps_spd_2d")
    c2.metric("Score", f"{score:.2f}")
    c3.metric("Verdict", verdict_text)
    c4.metric("Red Violations", red_violation_count)

    if verdict_text == "SAFE":
        st.success("✅ Overall driving is SAFE.")
    elif verdict_text == "MODERATE_RISK":
        st.warning("⚠️ Driving has MODERATE risk. Improve compliance.")
    else:
        st.error("🛑 Driving is UNSAFE. Please drive carefully.")
    ...
    st.subheader("Tilt Summary")
    t1, t2, t3, t4 = st.columns(4)
    t1.metric("High Pitch Events", high_pitch_events)
    t2.metric("High Roll Events", high_roll_events)
    t3.metric("Max |Pitch|", "Saved in previous run")
    t4.metric("Max |Roll|", "Saved in previous run")
    ...
    st.write("---")
    st.subheader("Suggestions")
    st.info("Previously processed results loaded from saved files.")

    st.write("---")
    st.subheader("Detected Stable Speed Sign Events")
    if len(stable_sign_df) == 0:
        st.warning("No stable speed sign events were detected.")
    else:
        st.dataframe(stable_sign_df, use_container_width=True, hide_index=True)
    ...
    # -------------------------
    # traffic sign visibility audit report
    # -------------------------
    if os.path.exists(report_csv):
        st.write("---")
        st.header("Traffic Sign Visibility Audit Report")
        ...
        st.subheader("Audit Summary")
        total_signs = len(report_df)
        signs_with_issues = int((report_df["issue"].astype(str) != "clear_visible").sum()) if "issue" in report_df.columns else 0
        high_severity = int((report_df["severity"].astype(str).str.lower() == "high").sum()) if "severity" in report_df.columns else 0
        unique_sign_types = int(report_df["sign_type"].astype(str).nunique()) if "sign_type" in report_df.columns else 0
        ...
        show_cols = [
            c for c in [
                "track_id", "frame", "sign_type", "speed_value", "issue",
                "severity", "priority", "latitude", "longitude",
                "maps_link", "recommended_action", "summary"
            ] if c in report_df.columns
        ]
        st.dataframe(report_df[show_cols], use_container_width=True, hide_index=True)

        # unique problem signs
        st.subheader("Problem Sign Summary")
